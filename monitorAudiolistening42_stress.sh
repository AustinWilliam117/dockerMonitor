...
AUDIO3log_path=$basePath/audiolistening_arm/audio_3/logs
AUDIO4log_path=$basePath/audiolistening_arm/audio_4/logs
AUDIO5log_path=$basePath/audiolistening_arm/audio_5/logs
...
do
    ...
    if [ "$beforeTime" != "$afterTime" ]; then
        echo ""
        echo "beforeTime时间是: $beforeTime, afterTime时间是: $afterTime, 正在统计$beforeTime日志"
        echo ""
        # 解压小程序日志
        ...
        echo ""
        echo "beforeTime时间是: $beforeTime, afterTime时间是: $afterTime, $beforeTime日志统计结束"
        echo ""
    else
        echo ""
        echo "beforeTime时间是: $beforeTime, afterTime时间是: $afterTime, 所以不需要统计前一天日志"
        echo ""
    fi
    ...
    #End_time=`date "+%Y-%m-%d_%H_%M_%S"`
    echo "jmeter执行程序内部结束时间为：$stressEndTime" 
    echo ""
    echo ""
